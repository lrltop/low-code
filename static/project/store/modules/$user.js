...
export default {
	...
	state: {
		/**
		 * js或页面直接调用示例
		 * ue.vuex.get('$user.userInfo');
		 * js更新示例
		 * ue.vuex.set('$user.userInfo.avatar', avatar);
		 */
		whelk_token: lifeData.$user.whelk_token,
		userInfo: lifeData.$user.userInfo || {},
		token: lifeData.$user.token,
	},
	/**
	...
	getters: {
		...
		hasLogin: state => state.token?true:false,
		/**
		 * 获取用户信息（使用计算属性获取）
		 * js调用示例
		 * ue.vuex.getters('$user/getUserInfo');
		 * 或
